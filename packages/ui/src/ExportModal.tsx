  ...
  const generateSVGString = (isTransparent: boolean) => {
    ...
    const svgElements = drawing.objects.map(obj => {
      switch (obj.type) {
        ...
        case 'text':
          return `<text x="${obj.x}" y="${obj.y}" fill="${obj.stroke === 'transparent' ? '#000000' : obj.stroke}" font-family="Geist, sans-serif" font-size="16" font-weight="500" dominant-baseline="middle">${obj.text || ''}</text>`;
        case 'image':
